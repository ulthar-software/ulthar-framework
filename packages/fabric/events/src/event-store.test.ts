...
import { WritableValueStore } from "@fabric/db";
import { Field } from "@fabric/models";
...
describe("EventStore", () => {
  ...
  beforeEach(async () => {
    const eventStorageDriver = new SQLiteStoreDriver(":memory:");
    const stateStorageDriver = new SQLiteStoreDriver(":memory:");
  ...
  test("given an event store, we can ");
});

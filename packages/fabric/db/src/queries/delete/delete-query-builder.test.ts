...
describe("StoreDeleteQueryBuilder", () => {
  ...
  test("given a query, when `manyWhere` is called, it should return an Effect", async () => {
    ...
    const query = new StoreDeleteQueryBuilder<Demo>(
      driver,
      Demo,
      "demo",
    ).manyWhere({ name: "test" });
  ...
  test("given a query, when `oneById` is called, it should return an Effect", async () => {
    const driver = partialMock<ValueStoreDriver>({
      delete: () => Effect.ok(),
    });
  ...
  test("given a query, when `all` is called, it should return an Effect", async () => {
    const driver = partialMock<ValueStoreDriver>({
      delete: () => Effect.ok(),
    });

    const query = new StoreDeleteQueryBuilder<Demo>(driver, Demo, "demo").all();

...
describe("Decimal", () => {
  ...
  test("toString", () => {
    const vectors = [
      ...
      { input: Decimal.from(12.3), output: "12.3" },
  ...
  test("fromFraction", () => {
    const vectors: ({ numerator: bigint; denominator: bigint } & (
      | { success: true; output: Decimal }
      | { success: false }
    ))[] = [
      {
        numerator: 1n,
    ...
    for (const vector of vectors) {
      ...
      if (vector.success) {
        ...
        for (const { numerator, denominator } of vectors) {
          expect(() =>
            Decimal.fromFraction({ numerator, denominator }),
          ).toThrow(Error);
        }
      }
    }
  ...
  test("div", () => {
    const vectors: ({ a: Decimal; b: Decimal; significantDigits?: number } & (
      | { success: true; output: Decimal }
      | { success: false }
    ))[] = [
      {
  ...
  test("inv", () => {
    const vectors: ({ input: Decimal; significantDigits?: number } & (
      | { success: true; output: Decimal }
      | { success: false }
    ))[] = [
  ...
  test("pow", () => {
    const vectors: ({ a: Decimal; b: number } & (
      | { success: true; output: Decimal }
      | { success: false }
    ))[] = [
  ...
  test("toBigInt", () => {
    const vectors: ({ input: Decimal } & (
      | { success: true; output: bigint }
      | { success: false }
    ))[] = [

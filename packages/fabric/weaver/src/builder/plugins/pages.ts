...
import type { RoutePath, RoutesDefinition } from "../../routing/route-path.js";

export const pagesPlugin = (routes: RoutesDefinition) =>
  ({
    name: "pages",
    setup(build) {
      build.onLoad({ filter: /routes.ts$/, namespace: "file" }, () => {
...
export function transformRoutes(routes: RoutesDefinition): RoutesDefinition {
  return Object.entries(routes).reduce<RoutesDefinition>(
    (acc, [route, path]) => {
...
export function routesToJsExtension(
  ...
  return Object.entries(routes).reduce<RoutesDefinition>(
    (acc, [route, path]) => {
      acc[route] = ("/" + path.slice(4).replace(".js", ".js")) as RoutePath;
      return acc;
    },
    {},
  );

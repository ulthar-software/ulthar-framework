...
import { UserUnenrolledEvent } from "../../models/enrollment.js";
import { UserInviteRemovedEvent } from "../../models/user-invite.js";
...
export const CancelInviteInputModel = new Schema({
  inviteId: Field.uuid(),
});
export type CancelInviteInput = Infer<typeof CancelInviteInputModel>;
...
// Define the use case
export const CancelInviteUseCase = new UseCase({
  name: "cancelInvite",
  type: "command",
  auth: AccessPolicy.WithPermission(Permission.INVITE_USERS),
  inputSchema: CancelInviteInputModel,
  effect: (
    { state, events, crypto, currentUser }: CancelInviteDependencies,
    { inviteId }: CancelInviteInput,
